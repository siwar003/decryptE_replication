...
INPUT_FILE    = "C:/Users/siwar/Desktop/drc_python/proteinGroups_fdr0.01.txt"
MAPPING_FILE  = "C:/Users/siwar/Desktop/drc_python/Mapping_Sheet.xlsx"
...
# ── Find plate number from the 'sample' column ──────────────
plate_num = None
for row in mapping_rows:
    s = str(row[col_sample]) if row[col_sample] else ""
    # Match drug name in sample (case-insensitive)
    if re.search(rf'(?i)_{re.escape(DRUG_NAME)}_', s):
        m = re.search(r'(?i)plate(\d+)', s)
        if m:
...
if plate_num is None:
    raise SystemExit(
        f"Could not find plate number for drug '{DRUG_NAME}' in the sample column."
    )
